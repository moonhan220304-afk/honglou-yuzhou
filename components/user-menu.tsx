    ...
    <div className="relative">
      <div className="flex items-center overflow-hidden rounded-full bg-surface-warm shadow-card">
        ...
        >
          ...
          {unread > 0 && (
            <span className="absolute right-0.5 top-0.5 flex h-3.5 min-w-3.5 items-center justify-center rounded-full bg-seal px-1 text-[9px] font-bold leading-none text-white">
              {unread > 99 ? "99+" : unread}
            </span>
          )}
        </Link>
      ...
      {open && (
        <div className="absolute right-0 top-10 z-50 w-40 overflow-hidden rounded-2xl border border-line/60 bg-surface p-1.5 shadow-hover">
          ...
          >
            <span>通知</span>
            {unread > 0 && (
              <span className="rounded-full bg-seal px-1.5 py-0.5 text-[10px] font-bold text-white">
                {unread > 99 ? "99+" : unread}
              </span>
            )}
          </Link>
